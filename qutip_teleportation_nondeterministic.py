...
def reconstruct(state, i):
    if i==0:
        ...
        state = sigmaz()*state
    return state
for i_initial, initial_state in enumerate([basis(2,0), basis(2,1)]):
#   Make three-particle state
    ...
    for outcome, psi in enumerate([phi_plus, phi_minus, psi_plus, psi_minus]):
        proj = tensor(psi.proj(), qeye(2))
        p, psi_123 = measure_observable(psi_23, proj)
        if p > 0:
            ...
            final = reconstruct(final, outcome)                    
            # Verify that Bob successfully reconstructed the state of particle one
            print(f'{i_initial} {outcome}')
            assert ( same_state(initial_state.full()[:,0], final.full()[:,0]))
